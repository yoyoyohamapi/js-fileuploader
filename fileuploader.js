(function (window) {
    ...
    // 文件状态
    var STATUS_WAIT = 0, // 等待上传
        STATUS_PENDING = 1, // 上传中
        STATUS_FINISHED = 2, // 上传完成
        STATUS_REMOVED = 3; //已删除
    ...
    function FileUploader(config) {
        this._config = extend(config, BASE_CONFIG);
        if (this._config.sharedSize > MAX_SHARED_SIZE)
            this._config.sharedSize = MAX_SHARED_SIZE;
        else if (this._config.sharedSize < MIN_SHARED_SIZE)
            this._config.sharedSize = MIN_SHARED_SIZE;
        var input = document.querySelector(this._config.selector);
        ...
        this._input.addEventListener('change', function (e) {
            ...
            // 刷新文件列表
            if (isFunction(this._config.onAppend)) {
                this._config.onAppend(info);
            }
        }.bind(this));
    }
    ...
    FileUploader.prototype.start = function () {
        ...
        this._send();
        // 开始以后的回调
        var onStart = this._config.onStart;
    ...
    FileUploader.prototype._send = function () {
        ...
        if (this._current) {
            ...
            for (var i = 0, length = this._files.length; i < length; i++) {
    ...
    FileUploader.prototype._createRequest = function (file, sharedSize, sharedCount) {
        var self = this,
            progressEach = this._config.progressEach,
            progressAll = this._config.progressAll,
            error = this._config.error,
            sendTo = this._config.url;
        // 初始化XMLHttpRequest对象
        ...
        xhr.onreadystatechange = function () {
            ...
            if (this.readyState === 0) {
                ...
                if (file.succeed === sharedCount)
                    self._completedSize += file.file.size - (sharedCount - 1) * sharedSize;
                else
                    self._completedSize += sharedSize;
                // 是否需要回调进度显示,
                ...
                if (isFunction(progressAll)) {
                    progressAll(self._completedSize, self._totalSize);
                }
    ...
    FileUploader.prototype._sendFile = function (file) {
        ...
        if (this._pausing)
            return;
        var sharedSize = this._config.sharedSize,
    ...
    FileUploader.prototype._sendMerge = function (file) {
        if (this._pausing)
            return;
        var url = this._config.mergeUrl || this._config.url,
            hash = file.hash,
            count = file.sharedCount,
            progressEach = this._config.progressEach,
            progressAll = this._config.progressAll,
            success = this._config.success,
            error = this._config.error,
            self = this;
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            if (this.readyState === 4 && (this.status < 300 || this.status === 302)) {
                ...
                // 当前待上传文件移位
                self._current = self._files[file.index + 1];
                // 标识当前文件已完成
    ...
    FileUploader.prototype._appendFiles = function (files) {
        ...
        outer:
            for (var i = 0; i < files.length; i++) {
                ...
                file = {
                    ...
                    hash: hash,
                    succeed: 0, // 该文件完成片数,
                    sharedCount: Math.ceil(file.size / this._config.sharedSize),// 该文件的总片数
        ...
        this._files = src.concat(wrapped);
        return {
            totalSize: totalSize,
            newFiles: wrapped
        };
    ...
    FileUploader.prototype.pause = function () {
        this._pausing = true;
        var onPause = this._config.onPause;
        if (isFunction(onPause)) {
            // 暂停回调提供一个当前暂停的文件
            onPause(this._current);
        }
    };
    ...
    /**
     * 删除文件
     * @param index 待删除文件在序列中的索引
     */
    FileUploader.prototype.remove = function (index) {
        // 删除的时候是否进入暂停模式
        ...
        file.status = STATUS_REMOVED;
        // 更新总大小
        this._totalSize = this._totalSize - file.file.size;
        // 删除完成的回调
        var onRemove = this._config.onRemove;
